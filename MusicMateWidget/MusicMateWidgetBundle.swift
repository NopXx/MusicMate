...
let widgetAppGroupID = "group.com.nopxx.MusicMate"
...
struct NowPlayingWidget: Widget {
    var body: some WidgetConfiguration {
        ...
        .description("Shows the current track from Apple Music via MusicMate.")
...
@MainActor
private func dispatchMusicMateCommand(_ command: String) {
    DistributedNotificationCenter.default().postNotificationName(
        NSNotification.Name("com.nopxx.MusicMate.WidgetCommand"),
        object: command,
        userInfo: nil,
...
struct PlayPauseIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        dispatchMusicMateCommand(isPlaying ? "pause" : "play")
        return .result()
    }
}
...
struct PreviousIntent: AppIntent {
    ...
    @MainActor
    func perform() async throws -> some IntentResult {
        dispatchMusicMateCommand("previous")
        return .result()
    }
}